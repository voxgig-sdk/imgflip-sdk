...
boru-driven command-line client **and** interactive REPL for the Imgflip
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/imgflip-cli)
make build
...
./imgflip-cli load 1 free            # {id:1} shorthand
./imgflip-cli load '{id:1}' free       # explicit match map

# 5. Override the API base URL for a single call
IMGFLIP_BASE=https://api.example.com ./imgflip-cli load 1 free
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/imgflip-cli
   ```
...
```sh
./imgflip-cli load 1 free          # scalar shorthand for {id:1}
./imgflip-cli load '{id:1}' free     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `imgflip>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `free`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/imgflip-cli`. |
...
(`./imgflip-cli <expr>`) and interactively (the REPL), and expressions compose the
