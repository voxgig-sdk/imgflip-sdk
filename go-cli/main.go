// Command Imgflip-cli is a CLI + REPL for the Imgflip SDK,
...
const prompt = "imgflip"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "free premium"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      IMGFLIP_APIKEY (api key), IMGFLIP_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: IMGFLIP_APIKEY carries the API key and
	// IMGFLIP_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("IMGFLIP_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("IMGFLIP_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewImgflipSDK(opts)
