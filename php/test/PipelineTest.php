...
{
    private static function utility(): ImgflipUtility
    {
        return new ImgflipUtility();
    }
    ...
    private static function ctx(array $over = []): ImgflipContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new ImgflipContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new ImgflipOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof ImgflipError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new ImgflipOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = ImgflipMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new ImgflipOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = ImgflipMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new ImgflipOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = ImgflipMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new ImgflipSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = ImgflipMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new ImgflipResponse([]),
            'result' => new ImgflipResult([]),
        ]);
        [, $err] = ImgflipMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new ImgflipSpec([]),
            'response' => null,
            'result' => new ImgflipResult([]),
        ]);
        [, $err] = ImgflipMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new ImgflipSpec(['step' => 's']),
            'response' => new ImgflipResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new ImgflipResult([]),
        ]);
        [, $err] = ImgflipMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new ImgflipSpec(['step' => 's']),
            'response' => new ImgflipResponse(self::resp(200, ['v' => 1])),
            'result' => new ImgflipResult([]),
        ]);
        [, $err] = ImgflipMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new ImgflipSpec(['step' => 's']),
            'response' => new ImgflipResponse(self::resp(200, ['v' => 2])),
            'result' => new ImgflipResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        ImgflipMakeResponse::call($ctx);
    ...
    {
        $preset = new ImgflipResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new ImgflipSpec([]),
            'response' => new ImgflipResponse([]),
            'result' => new ImgflipResult([]),
        ...
        [$got, $err] = ImgflipMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new ImgflipResult([])]);
        [, $err] = ImgflipMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new ImgflipSpec([]), 'result' => null]);
        [, $err] = ImgflipMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new ImgflipSpec(['step' => 's']),
            'result' => new ImgflipResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new ImgflipOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = ImgflipMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new ImgflipSpec(['step' => 's']),
            'result' => new ImgflipResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new ImgflipOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = ImgflipMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new ImgflipResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new ImgflipSpec([]),
            'result' => new ImgflipResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = ImgflipMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = ImgflipMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new ImgflipError('boom', 'boom');
        $utility->fetcher = function (ImgflipContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new ImgflipSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = ImgflipMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (ImgflipContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = ImgflipMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(ImgflipResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (ImgflipContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new ImgflipSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = ImgflipMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new ImgflipResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new ImgflipSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = ImgflipMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = ImgflipMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = ImgflipMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new ImgflipResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, ImgflipDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new ImgflipResult(['ok' => false])]);
        $this->expectException(ImgflipError::class);
        ImgflipDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new ImgflipResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', ImgflipMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        ImgflipMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new ImgflipBaseFeature();
        $b = new ImgflipBaseFeature();
        ImgflipFeatureAdd::call($ctx, $a);
        ImgflipFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): ImgflipBaseFeature
    {
        $f = new ImgflipBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        ImgflipFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): ImgflipContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new ImgflipSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = ImgflipPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new ImgflipResponse(['headers' => null]),
            'result' => new ImgflipResult([]),
        ]);
        ImgflipResultHeaders::call($ctx);
