...
class FreeEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new FreeEntity($this->_client, $opts);
    }

    /**
     * @param Free|array $args Free data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = ImgflipHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Free|array The current Free data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Free fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = ImgflipHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Free fields).
     */
     ...
     * Load a single Free.
     *
     * @param FreeLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed FreeLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Free|array The loaded Free as an assoc-array at the
     *   SDK boundary; throws ImgflipError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = ImgflipHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
     ...
     * Create a new Free.
     *
     * @param FreeCreateData|array|null $reqdata Body data as an assoc-array;
     *   a typed FreeCreateData names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Free|array The created Free as an assoc-array at the
     *   SDK boundary; throws ImgflipError on failure (item-5 convention).
     */
