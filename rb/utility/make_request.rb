...
module ImgflipUtilities
  MakeRequest = ->(ctx) {
    ...
    if fetch_err
      ...
      response = ImgflipResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = ImgflipResponse.new(fetched)
    else
