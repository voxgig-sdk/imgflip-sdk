...
class FreeEntity
  ...
  def make
    opts = @_entopts.dup
    FreeEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = ImgflipHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Free, Hash] the current Free data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = ImgflipHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Free fields)
  def match_get
  ...
  # Load a single Free.
  #
  # @param reqmatch [FreeLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Free.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Free, Hash] the loaded Free; raises ImgflipError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = ImgflipHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  # Create a new Free.
  #
  # @param reqdata [FreeCreateData, Hash, nil] body data
  # @param ctrl [Object, nil] optional per-call control
  # @return [Free, Hash] the created Free; raises ImgflipError on failure
