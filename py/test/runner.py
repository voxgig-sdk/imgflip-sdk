...
class ImgflipTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            ImgflipTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = ImgflipTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = ImgflipTestRunner.getenv("IMGFLIP_TEST_LIVE")
        override = ImgflipTestRunner.getenv("IMGFLIP_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = ImgflipTestRunner.getenv(key)
        ...
        explain = ImgflipTestRunner.getenv("IMGFLIP_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["IMGFLIP_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if ImgflipTestRunner._test_control is not None:
            return ImgflipTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                ImgflipTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            ImgflipTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = ImgflipTestRunner.load_test_control()
...
def load_env_local():
    ImgflipTestRunner.load_env_local()


def env_override(m):
    return ImgflipTestRunner.env_override(m)


def entity_data(v):
    return ImgflipTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return ImgflipTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return ImgflipTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return ImgflipTestRunner.load_test_control()


def live_client_options():
    return ImgflipTestRunner.live_client_options()


def live_delay_ms():
    return ImgflipTestRunner.live_delay_ms()
