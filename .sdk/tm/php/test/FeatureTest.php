...
class FtCtrl extends ImgflipControl
{
...
class FtClient
{
    ...
    private ImgflipUtility $utility;

    public function __construct(ImgflipUtility $utility, array $options)
    {
    ...
    public function get_utility(): ImgflipUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (ImgflipContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (ImgflipContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = ImgflipConfig::make_feature($name);
    ...
    private static function buildUrl(ImgflipSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof ImgflipError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new ImgflipResponse($fetched);
        }
    ...
    private function fail(ImgflipContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (ImgflipContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
