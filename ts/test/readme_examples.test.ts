...
//     (\`new ImgflipSDK(...)\` / \`ImgflipSDK.test(...)\` become
//     \`ImgflipSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"free":{"test01":{"id":"test01"}},"premium":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new ImgflipSDK(...)\` and any
// \`ImgflipSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+ImgflipSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/ImgflipSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
