...
ImgflipSDK._make_feature = _make_feature


function ImgflipSDK.new(options)
  local self = setmetatable({}, ImgflipSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function ImgflipSDK:options_map()
...
function ImgflipSDK:get_utility()
  return Utility.copy(self._utility)
end


function ImgflipSDK:get_root_ctx()
  return self._rootctx
end


function ImgflipSDK:prepare(fetchargs)
...
function ImgflipSDK:direct(fetchargs)
...
-- by passing it.
function ImgflipSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function ImgflipSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "ImgflipSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function ImgflipSDK:Free(data)
  ...
  if data == nil then
    if self._free == nil then
      self._free = EntityMod.new(self, nil)
    end
    return self._free
  end
...
function ImgflipSDK:Premium(data)
  ...
  if data == nil then
    if self._premium == nil then
      self._premium = EntityMod.new(self, nil)
    end
...
function ImgflipSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = ImgflipSDK.new(sdkopts)
  sdk.mode = "test"
