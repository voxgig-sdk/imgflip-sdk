...
The Lua SDK for the Imgflip API — an entity-oriented client using Lua conventions.

It exposes the API as capitalised, semantic **Entities** — e.g. `client:Free()` — each with the same small set of operations (`load`, `create`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
```
lua/
├── imgflip_sdk.lua    -- Main SDK module
...
The main module (`imgflip_sdk`) exports the SDK constructor
