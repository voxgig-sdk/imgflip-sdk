...
local FreeEntity = {}
FreeEntity.__index = FreeEntity


function FreeEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, FreeEntity)
  self._name = "free"
  self._client = client
...
function FreeEntity:get_name()
  return self._name
end


function FreeEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return FreeEntity.new(self._client, opts)
end
...
function FreeEntity:mark_deleted()
  self._deleted = true
end


function FreeEntity:deleted()
  return true == self._deleted
end


function FreeEntity:data_set(args)
  if args ~= nil then
...
function FreeEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function FreeEntity:match_set(args)
  if args ~= nil then
...
function FreeEntity:stream(action, args, callopts)
...
function FreeEntity:_run_op(ctx, post_done)
  local utility = self._utility
