...
local PremiumEntity = {}
PremiumEntity.__index = PremiumEntity


function PremiumEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, PremiumEntity)
  self._name = "premium"
  self._client = client
...
function PremiumEntity:get_name()
  return self._name
end


function PremiumEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return PremiumEntity.new(self._client, opts)
end
...
function PremiumEntity:mark_deleted()
  self._deleted = true
end


function PremiumEntity:deleted()
  return true == self._deleted
end


function PremiumEntity:data_set(args)
  if args ~= nil then
...
function PremiumEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PremiumEntity:match_set(args)
  if args ~= nil then
...
function PremiumEntity:match_get()
...
function PremiumEntity:stream(action, args, callopts)
...
function PremiumEntity:_run_op(ctx, post_done)
  local utility = self._utility
