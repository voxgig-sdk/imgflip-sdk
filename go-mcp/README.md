...
[MCP](https://modelcontextprotocol.io) server exposing the Imgflip SDK as
two agent tools — `imgflip_list` and `imgflip_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/imgflip-mcp)
make build

# 2. Provide credentials via the environment
export IMGFLIP_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user imgflip \
  -- /absolute/path/to/imgflip-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./imgflip-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// imgflip_list: first page of records
{ "entity": "free" }
{ "entity": "free", "query": { } }

// imgflip_load: one record by id
{ "entity": "free", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/imgflip-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user imgflip \
     -- "$PWD"/dist/*/imgflip-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `imgflip_list` and `imgflip_load` tools now appear
   in new sessions. Ask the agent to *"list free using imgflip"*
   and it calls `imgflip_list` with `{"entity":"free"}`.
...
```sh
export IMGFLIP_APIKEY=sk_live_xxx            # API key
export IMGFLIP_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `imgflip_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `imgflip_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `IMGFLIP_APIKEY` | API key sent with every request. |
| `IMGFLIP_BASE` | Optional override of the API base URL. |
...
```sh
./imgflip-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"imgflip_load","arguments":{"entity":"free","query":{"id":1}}}}'
```
