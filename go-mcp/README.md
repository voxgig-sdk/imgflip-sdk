...
MCP server exposing the Imgflip SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `imgflip_list` | `entity`, optional `query` map | First page of records as JSON |
| `imgflip_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
After install, restart Claude Code; the `imgflip_list` and
`imgflip_load` tools become available in new sessions.
