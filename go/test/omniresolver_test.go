...
import (
	...
	omni "github.com/voxgig-sdk/imgflip-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.ImgflipError); is {
				out := map[string]any{
					"name":    "ImgflipError",
...
func omniCtx(arg any, client *sdk.ImgflipSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.ImgflipSDK); is {
			client = live
